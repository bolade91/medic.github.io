			...
			if($holidaysQry->rowCount()==0) {
				for($j=0;$j<count($resultTime);$j++) {
					...
					if($checkQry->rowCount()==0) {
                        ...
                        $query->execute(array($slot_special_text,$slot_special_mode,$resultDate[$i],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$price,$slot_av,$slot_av_max,1,$_POST["calendar_id"],$discount_price,$perc_price,$_POST["slot_show_price"]));
